        ...
        <div className="w-full max-w-md">
          <div className="rounded-3xl p-8 border border-white/[0.10]"
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: 'Google', provider: 'google' as const, color: '#f43f5e', icon: (
                ...
                { label: 'GitHub', provider: 'github' as const, color: '#a78bfa', icon: (
                ...
                <button key={s.label} onClick={() => handleOAuth(s.provider)}
                  disabled={oauthLoading !== null}
                  className="flex items-center justify-center gap-2 py-3 rounded-2xl text-sm font-medium text-white/60 hover:text-white/90 transition-all disabled:opacity-50"
                  style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}
                  onMouseEnter={e => { (e.currentTarget.style.borderColor = `${s.color}60`); (e.currentTarget.style.boxShadow = `0 0 16px ${s.color}20`); }}
                  onMouseLeave={e => { (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'); (e.currentTarget.style.boxShadow = 'none'); }}>
                  {oauthLoading === s.provider
