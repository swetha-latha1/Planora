...
const inputCls = 'px-4 py-2.5 rounded-xl text-sm text-white/80 placeholder-white/25 outline-none transition-all w-56 focus:ring-1 focus:ring-accent/50'
  + ' ' + 'border border-white/[0.1] focus:border-accent/60';
...
export default function ProfilePage() {
  ...
  const [email,    setEmail]    = useState('swetha@example.com');
  ...
  useEffect(() => {
    setName(localStorage.getItem('pd_name') || 'Swetha Sai');
    setEmail(localStorage.getItem('pd_email') || 'swetha@example.com');
    setBio(localStorage.getItem('pd_bio') || '');
    setAvatar(localStorage.getItem('pd_avatar') || '🧑‍💻');
    setTimezone(localStorage.getItem('pd_timezone') || 'Asia/Kolkata');
  }, []);

  const saveProfile = () => {
    localStorage.setItem('pd_name', name);
    localStorage.setItem('pd_email', email);
    localStorage.setItem('pd_bio', bio);
    localStorage.setItem('pd_avatar', avatar);
    localStorage.setItem('pd_timezone', timezone);
    setSaved(true);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5 animate-fade-in pb-10">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          ...
          <div className="flex-1 min-w-0 w-full">
            <div className="flex items-center gap-3 flex-wrap mb-1">
              <h2 className="text-xl font-bold text-white/90">{name || 'Your Name'}</h2>
            ...
            <p className="text-sm text-white/55 leading-relaxed">{bio || 'No bio yet — tell us about yourself.'}</p>
      ...
      <Section title="Personal Information" icon="✏️">
        ...
        <Row label="Email Address" desc="Used for account recovery">
          <input className={inputCls} style={{ background: '#1a1840' }} value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" type="email" />
        </Row>
        <Row label="Bio" desc="A short description about yourself">
          <textarea
            className={cn(inputCls, 'resize-none h-20 leading-relaxed')}
      ...
      <Section title="Account" icon="🔐">
        <Row label="Account Type" desc="Your current plan">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold px-3 py-1.5 rounded-full bg-accent/20 text-accent2 border border-accent/30">
              ⭐ Pro Plan
            </span>
          </div>
        </Row>
        <Row label="Member Since" desc="When you joined FocusOS">
          <span className="text-sm text-white/50 font-medium">January 2024</span>
        </Row>
        ...
        <Row label="User ID" desc="Your unique identifier">
          <span className="text-xs font-mono text-white/30 px-3 py-1.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
            usr_focusos_001
          </span>
        </Row>
      </Section>
      ...
      <Section title="Change Password" icon="🔑">
        ...
        {pwError && (
          <div className="px-6 py-2">
            <p className="text-xs text-rose-400 font-medium">{pwError}</p>
          </div>
        )}
        <div className="px-6 py-4 flex justify-end">
      ...
      <Section title="Danger Zone" icon="⚠️">
        <Row label="Delete Account" desc="Permanently delete your account and all data">
          <button
            onClick={() => confirm('Are you sure? This cannot be undone.') && alert('Account deletion requested.')}
