...
const SPARK_ZERO = [0,0,0,0,0,0,0].map(v => ({ v }));

export default function DashboardPage() {
  const user = useUser();
  const { tasks: { tasks, addTask, toggleTask } } = useData();
  ...
  const todayDone = tasks.filter(t => t.completed && t.createdAt?.startsWith(new Date().toISOString().slice(0, 10))).length;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-3 md:gap-4">
        <SummaryCard label="Productivity Score" value={completionRate} suffix="/100" delay={0} trend={{ value: 0, label: '% vs yesterday' }} spark={SPARK_ZERO} gradient="bg-gradient-to-br from-violet-600/20 to-accent2/10" glowColor="rgba(124,106,247,0.4)" iconBg="bg-gradient-to-br from-violet-500 to-accent2" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-5 h-5"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" strokeLinejoin="round"/></svg>} />
        <SummaryCard label="Active Habits" value={0} delay={100} trend={{ value: 0, label: ' new this week' }} spark={SPARK_ZERO} gradient="bg-gradient-to-br from-cyan-500/20 to-accent3/10" glowColor="rgba(6,182,212,0.4)" iconBg="bg-gradient-to-br from-cyan-400 to-accent3" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-5 h-5"><path d="M12 2a10 10 0 100 20A10 10 0 0012 2z"/><path d="M12 6v6l4 2"/></svg>} />
        <SummaryCard label="Longest Streak" value={0} suffix=" days" delay={200} trend={{ value: 0, label: ' days gained' }} spark={SPARK_ZERO} gradient="bg-gradient-to-br from-orange-500/20 to-rose-500/10" glowColor="rgba(249,115,22,0.4)" iconBg="bg-gradient-to-br from-orange-400 to-rose-500" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-5 h-5"><path d="M8.5 14.5A2.5 2.5 0 0011 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 11-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 002.5 2.5z"/></svg>} />
        <SummaryCard label="Monthly Completion" value={completionRate} suffix="%" delay={300} trend={{ value: 0, label: '% vs last month' }} spark={SPARK_ZERO} gradient="bg-gradient-to-br from-emerald-500/20 to-teal-500/10" glowColor="rgba(16,185,129,0.4)" iconBg="bg-gradient-to-br from-emerald-400 to-teal-500" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-5 h-5"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><path d="M22 4L12 14.01l-3-3"/></svg>} />
        <SummaryCard label="Tasks Done Today" value={todayDone} delay={400} trend={{ value: 0, label: ' more than avg' }} spark={SPARK_ZERO} gradient="bg-gradient-to-br from-pink-500/20 to-rose-500/10" glowColor="rgba(236,72,153,0.4)" iconBg="bg-gradient-to-br from-pink-500 to-rose-500" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-5 h-5"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18M9 16l2 2 4-4"/></svg>} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4 md:gap-5">
        <div className="lg:col-span-2 glass rounded-3xl p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-5">
            {[
              ...
              <div key={s.label} className="flex flex-col items-center justify-center py-3 px-2 rounded-2xl transition-all" style={{ background: s.bg, border: `1px solid ${s.border}` }}>
        ...
        <div className="glass rounded-3xl p-5 flex flex-col items-center justify-center gap-4">
          ...
          <div className="w-full space-y-2">
            {[
              { label: 'Work',     val: tasks.filter(t => t.completed && t.category === 'work').length > 0 ? Math.round(tasks.filter(t => t.completed && t.category === 'work').length / Math.max(tasks.filter(t => t.category === 'work').length, 1) * 100) : 0, color: 'from-accent to-accent2' },
              { label: 'Personal', val: tasks.filter(t => t.completed && t.category === 'personal').length > 0 ? Math.round(tasks.filter(t => t.completed && t.category === 'personal').length / Math.max(tasks.filter(t => t.category === 'personal').length, 1) * 100) : 0, color: 'from-accent3 to-accent' },
              { label: 'Health',   val: tasks.filter(t => t.completed && t.category === 'health').length > 0 ? Math.round(tasks.filter(t => t.completed && t.category === 'health').length / Math.max(tasks.filter(t => t.category === 'health').length, 1) * 100) : 0, color: 'from-emerald-500 to-accent3' },
            ].map(item => (
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
        <div className="glass rounded-3xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/projects" className="text-xs text-accent2 hover:text-white transition-colors flex items-center gap-1">
              View all →
            </Link>
          </div>
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-xl flex items-center justify-center text-base bg-gradient-to-br from-violet-500 to-purple-400">🚀</div>
                <span className="text-sm text-white/70 font-medium">My First Project</span>
              </div>
              <span className="text-xs text-white/30">0 tasks</span>
            </div>
            <div className="h-1.5 progress-track">
              <div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-purple-400 transition-all duration-700" style={{ width: '0%' }} />
            </div>
            <p className="text-[10px] text-white/25 mt-1">0% complete — just getting started!</p>
          </div>

          {/* Add New Project button → redirects to /projects */}
          <Link href="/projects"
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-2xl text-sm font-semibold text-white/60 hover:text-white border border-dashed border-white/[0.12] hover:border-accent/40 hover:bg-accent/5 transition-all">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-4 h-4">
              <path d="M12 5v14M5 12h14"/>
            </svg>
            Add New Project
        ...
        <div className="glass rounded-3xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-accent2 cursor-pointer hover:text-white transition-colors">See all</span>
          </div>
          {recent.length === 0 ? (
            <div className="flex flex-col gap-3">
              {!showForm ? (
                ...
                <form onSubmit={submitTask} className="flex flex-col gap-3 animate-fade-in">
                  <input autoFocus className="w-full px-4 py-2.5 rounded-xl text-sm text-white/80 placeholder-white/25 outline-none border border-white/[0.1] focus:border-accent/60 transition-all" style={{ background: 'rgba(255,255,255,0.05)' }} placeholder="Task title *" value={title} onChange={e => setTitle(e.target.value)} required />
                  <div className="grid grid-cols-2 gap-2">
                    <select className="px-3 py-2.5 rounded-xl text-sm text-white/70 outline-none border border-white/[0.1] focus:border-accent/60 transition-all cursor-pointer" style={{ background: '#1a1030' }} value={priority} onChange={e => setPriority(e.target.value as Priority)}>
                      <option value="high" style={{ background: '#1a1030' }}>🔴 High</option>
                      <option value="medium" style={{ background: '#1a1030' }}>🟡 Medium</option>
                      <option value="low" style={{ background: '#1a1030' }}>🟢 Low</option>
                    </select>
                    <select className="px-3 py-2.5 rounded-xl text-sm text-white/70 outline-none border border-white/[0.1] focus:border-accent/60 transition-all cursor-pointer" style={{ background: '#1a1030' }} value={category} onChange={e => setCategory(e.target.value as Category)}>
                      <option value="work" style={{ background: '#1a1030' }}>💼 Work</option>
                      <option value="personal" style={{ background: '#1a1030' }}>👤 Personal</option>
                      <option value="health" style={{ background: '#1a1030' }}>❤️ Health</option>
                      <option value="other" style={{ background: '#1a1030' }}>📌 Other</option>
                    </select>
                  </div>
                  <input type="date" className="w-full px-4 py-2.5 rounded-xl text-sm text-white/70 outline-none border border-white/[0.1] focus:border-accent/60 transition-all" style={{ background: 'rgba(255,255,255,0.05)', colorScheme: 'dark' }} value={dueDate} onChange={e => setDueDate(e.target.value)} />
              ...
              {tasks.slice(0, 5).map((t, i) => (
                <div key={t.id} className="flex gap-3 group">
                  <div className="flex flex-col items-center gap-1 shrink-0">
                    <div className={`w-2 h-2 rounded-full mt-1.5 ${t.completed ? 'bg-emerald-400' : t.priority === 'high' ? 'bg-rose-400' : 'bg-accent2'}`} />
                    {i < Math.min(tasks.length, 5) - 1 && <div className="w-px flex-1 bg-white/[0.06]" />}
                  </div>
                  <div className="pb-3 flex-1 min-w-0">
                    <p className="text-xs text-white/60 group-hover:text-white/80 transition-colors leading-relaxed">
                      {t.completed ? `✅ Completed "${t.title}"` : `📌 Added "${t.title}"`}
                    </p>
                    <p className="text-[10px] text-white/25 mt-0.5">{t.createdAt ? new Date(t.createdAt).toLocaleDateString() : ''}</p>
                  </div>
                </div>
              ))}
            </div>
...
function CompletionRing({ percent }: { percent: number }) {
  ...
  return (
    <svg width={size} height={size}>
      ...
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="url(#ringGrad)" strokeWidth={stroke} strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={circ * (1 - percent / 100)} transform={`rotate(-90 ${size/2} ${size/2})`} style={{ transition: 'stroke-dashoffset 0.8s cubic-bezier(0.4,0,0.2,1)' }}/>
